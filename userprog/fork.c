...
static int32_t copy_pcb_vaddrbitmap_stack0(struct task_struct *child_thread, struct task_struct *parent_thread){
	/*a.复制pcb所在的整个页，里面包含进程pcb信息及特权0级的栈，包含了返回地址*/
		...
		if(vaddr_btmp[idx_byte]){
			...
			while(idx_bit < 8){
				if( (BITMAP_MASK << idx_bit) & vaddr_btmp[idx_byte] ){
					...
					/*下面的操作是将父进程用户空间中的数据通过内核空间做中专，最终复制到子进程的空间中*/
					/*将父进程在用户空间中的数据复制到内核缓冲区buf_page
					 * 目的是下面切换到子进程的页表后，还能访问到父进程的数据*/
					memcpy(buf_page, (void *)prog_vaddr, PG_SIZE);
					/*将页表切换到子进程，目的是避免下面申请内存的函数
					 * 将pte及pde安装在父进程的页表中*/
...
pid_t sys_fork(void){
	...
	/*添加到就绪线程队列和所有线程队列，子线程由调试器安排运行*/
	ASSERT(elem_find(&thread_ready_list, &child_thread->general_tag) == 0);
	list_append(&thread_ready_list, &child_thread->general_tag);
	ASSERT(elem_find(&thread_all_list, &child_thread->all_list_tag) == 0);
